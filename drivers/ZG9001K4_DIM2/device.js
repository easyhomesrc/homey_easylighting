...
class RemoteControl extends ZigBeeRemoteControl {
    ...
    Object.keys(this.zclNode.endpoints).forEach((endpoint) => {
      ...
      this.zclNode.endpoints[endpoint].bind(CLUSTER.ON_OFF.NAME,
        new OnOffBoundCluster({
          onSetOff: this._onOffCommandHandler.bind(this, 'ZG9001K4_DIM2_off'),
          onSetOn: this._onOffCommandHandler.bind(this, 'ZG9001K4_DIM2_on'),
          endpoint: endpoint,
        }))
  ...
  _onLevelMoveWithOnOff ({ moveMode, rate }, endpoint) {
    ...
    const state = { 'group': endpoint }
    this.driver.getDeviceTriggerCard('ZG9001K4_DIM2_level_move_with_onoff').
  ...
  _onLevelStopWithOnOff (endpoint) {
    ...
    const tokens = {}
    const state = { 'group': endpoint }
    this.driver.getDeviceTriggerCard('ZG9001K4_DIM2_level_stop_with_onoff').
      trigger(this, tokens, state)
  }
