...
class RemoteControl extends ZigBeeRemoteControl {
    ...
    Object.keys(this.zclNode.endpoints).forEach((endpoint) => {
      ...
      this.zclNode.endpoints[endpoint].bind(CLUSTER.ON_OFF.NAME,
        new OnOffBoundCluster({
          onSetOff: this._onOffCommandHandler.bind(this, 'ZGRC_KEY_037_off'),
          onSetOn: this._onOffCommandHandler.bind(this, 'ZGRC_KEY_037_on'),
          endpoint: endpoint,
        }))
  ...
  _onRecallScene ({ groupId, sceneId }, endpoint) {
    ...
    const state = { 'group': endpoint }
    this.driver.getDeviceTriggerCard('ZGRC_KEY_037_recall_scene').
  ...
  _onStoreScene ({ groupId, sceneId }, endpoint) {
    ...
    const state = { 'group': endpoint }
    this.driver.getDeviceTriggerCard('ZGRC_KEY_037_store_scene').
      trigger(this, tokens, state)
  }
  ...
  _onLevelMoveWithOnOff ({ moveMode, rate }, endpoint) {
    ...
    const state = { 'group': endpoint }
    this.driver.getDeviceTriggerCard('ZGRC_KEY_037_level_move_with_onoff').
      trigger(this, tokens, state)
  }
  ...
  _onLevelStopWithOnOff (endpoint) {
    ...
    const tokens = {}
    const state = { 'group': endpoint }
    this.driver.getDeviceTriggerCard('ZGRC_KEY_037_level_stop_with_onoff').
      trigger(this, tokens, state)
  }
