...
class RemoteControl extends ZigBeeRemoteControl {
    ...
    Object.keys(this.zclNode.endpoints).forEach((endpoint) => {
      ...
      this.zclNode.endpoints[endpoint].bind(CLUSTER.ON_OFF.NAME,
        new OnOffBoundCluster({
          onSetOff: this._onOffCommandHandler.bind(this, 'ZG2833K4_EU06_off'),
          onSetOn: this._onOffCommandHandler.bind(this, 'ZG2833K4_EU06_on'),
          endpoint: endpoint,
        }))
  ...
  _onLevelMoveWithOnOff ({ moveMode, rate }, endpoint) {
    ...
    const state = { 'group': endpoint }
    this.driver.getDeviceTriggerCard('ZG2833K4_EU06_level_move_with_onoff').
      trigger(this, tokens, state)

    if (moveMode === 'up') {

      this.driver.getDeviceTriggerCard('ZG2833K4_EU06_move_level_up').
        trigger(this, { 'rate': tokens.rate }, state)

    } else if (moveMode === 'down') {

      this.driver.getDeviceTriggerCard('ZG2833K4_EU06_move_level_down').
        trigger(this, { 'rate': tokens.rate }, state)
    }
  }
  ...
  _onLevelStopWithOnOff (endpoint) {
    ...
    const tokens = {}
    const state = { 'group': endpoint }
    this.driver.getDeviceTriggerCard('ZG2833K4_EU06_level_stop_with_onoff').
      trigger(this, tokens, state)
  }
