...
const MAX_HUE = 254// 0xFE
const MAX_DIM = MAX_HUE
const MAX_SATURATION = MAX_HUE
const CIE_MULTIPLIER = 65536// 0x10000
  ...
  async onNodeInit ({
    ...
    if (this.hasCapability('dim')) {
      this.zclNode.endpoints[1].clusters.levelControl.on('attr.currentLevel', async value => {
        ...
        const diff = Math.abs(oldValue * MAX_DIM - value)
        console.log(`attr.currentLevel oldValue newValue diff`, oldValue, value, diff)
        if (diff >= 5) {
          let newValue = parseFloat((value / MAX_DIM).toFixed(2))
