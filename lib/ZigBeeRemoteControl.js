...
class ZigBeeRemoteControl extends ZigBeeDevice {
  ...
  async onNodeInit ({ zclNode, node }) {
    ...
    // Register measure_battery。读取当前当量百分比，通过总数计算成电量后返回
    this.registerCapability('measure_battery', CLUSTER.POWER_CONFIGURATION,
      {
        ...
        getOpts: {
          getOnStart: true,
          getOnOnline: true,
          pollInterval: 60 * 60 * 1000, // 30 minutes
        },
        reportParser: report => {
          this.log('reportParser => ' + JSON.stringify(report))
          let battery = Math.round(report * batteryMultiplier)
          return battery > 100 ? battery * 0.5 : battery
        },
      })
